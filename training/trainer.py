...
def print_stats(stats, phase):
    ...
    if "mean_pos_pair_dist" in stats:
        ...
        l += [
            stats["min_pos_pair_dist"],
            stats["mean_pos_pair_dist"],
            stats["max_pos_pair_dist"],
            stats["min_neg_pair_dist"],
            stats["mean_neg_pair_dist"],
            stats["max_neg_pair_dist"],
        ]
    ...
    if "final_loss" in stats:
        ...
        if "cloud_loss" in stats:
            ...
            s3 += "/cloud"
            s4 += "/ {:.1f}".format(
                stats["cloud_num_non_zero_triplets"],
            )
        if "image_loss" in stats:
            ...
            s3 += "/image"
            s4 += "/ {:.1f}".format(
                stats["image_num_non_zero_triplets"],
            )
    ...
    for epoch in tqdm.tqdm(range(1, params.epochs + 1)):
        ...
        if epoch % 1 == 0:
            ...
            for database_name in final_eval_stats:
                # print(minRecall, final_eval_stats[database_name]["ave_recall"][0])
                if minRecall < final_eval_stats[database_name]["ave_recall"][0]:
                    minRecall = final_eval_stats[database_name]["ave_recall"][0]
                    final_model_path = (
                        model_pathname + "_" + database_name + "_best.pth"
                    )
        ...
        if params.batch_expansion_th is not None:
            ...
            if "num_non_zero_triplets" in epoch_train_stats:
                ...
                rnz = (
                    epoch_train_stats["num_non_zero_triplets"]
                ...
                rnz = []
                rnz.append(
                    epoch_train_stats["final_num_non_zero_triplets"]
                    / epoch_train_stats["final_num_triplets"]
                )
                if "image_num_non_zero_triplets" in epoch_train_stats:
                    rnz.append(
                        epoch_train_stats["image_num_non_zero_triplets"]
                        / epoch_train_stats["image_num_triplets"]
                    )
                if "cloud_num_non_zero_triplets" in epoch_train_stats:
                    rnz.append(
                        epoch_train_stats["cloud_num_non_zero_triplets"]
                        / epoch_train_stats["cloud_num_triplets"]
                    )
                rnz = max(rnz)
                if rnz < params.batch_expansion_th:
                    dataloaders["train"].batch_sampler.expand_batch()
