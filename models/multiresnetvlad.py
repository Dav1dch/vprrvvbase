...
class MultiResNetVLADVGG16(nn.Module):
    """NetVLAD layer implementation"""
    ...
    def __init__(self, num_clusters=64, dim=512, normalize_input=True, vladv2=False):
        ...
        Args:
            ...
            vladv2 : bool
                If true, use vladv2 otherwise use vladv1
        """
        super(MultiResNetVLADVGG16, self).__init__()
        ...
        self.scaleSpace = [2, 3, 4, 5, 6, 7, 8, 9, 10]
    ...
    def forward(self, batch):
        ...
        for l in self.scaleSpace:
            input_scaled = input[:, :, ::l, ::l]
            image_scaled_encoding = self.model.encoder(input_scaled)
            image_encoding = torch.cat(
                [
                    image_encoding,
                    image_scaled_encoding.view(
                        image_scaled_encoding.size(0), image_scaled_encoding.size(1), -1
                    ),
                ],
                dim=2,
            )
        image_encoding = image_encoding.unsqueeze(3)
        vlad = self.model.vlad(image_encoding)
