    ...
    def __init__(
        ...
        if fuse_method == "concat":
            ...
            raise NotImplementedError(
                "Unsupported fuse method: {}".format(self.fuse_method)
            )
        ...
        if self.final_block is None:
            ...
            self.final_net = nn.Sequential(
                nn.Linear(self.fused_dim, temp_channels, bias=False),
...
class ResnetFPN(torch.nn.Module):
    def __init__(
        ...
        for i in range(
            self.fh_num_bottom_up - self.fh_num_top_down, self.fh_num_bottom_up
        ):
            self.fh_conv1x1[str(i + 1)] = nn.Conv2d(
                in_channels=layers[i], out_channels=self.lateral_dim, kernel_size=1
            )
            self.fh_tconvs[str(i + 1)] = torch.nn.ConvTranspose2d(
    ...
    def forward(self, batch):
        ...
        for i in range(
            self.fh_num_bottom_up, self.fh_num_bottom_up - self.fh_num_top_down, -1
        ):
